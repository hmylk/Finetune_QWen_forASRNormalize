...
if (( STAGE <= 1 && STOP_STAGE >= 1 )); then
  echo "[Stage 1] Train template LoRA"
  CUDA_VISIBLE_DEVICES="${GPU_LIST}" torchrun --nproc_per_node="${NPROC_PER_NODE}" train_template.py
fi

if (( STAGE <= 2 && STOP_STAGE >= 2 )); then
  echo "[Stage 2] Merge LoRA and run simple inference"
  RUN_MERGED_TEST=1 TEST_TEXT="${TEST_TEXT}" "${PYTHON_BIN}" merge_lora_template.py
...
if (( STAGE <= 3 && STOP_STAGE >= 3 )); then
  ...
  "${PYTHON_BIN}" export_to_llamacpp.py \
    --merged-model-dir ./asr_formal_merged \
    --gguf-output ./asr_formal_merged-llamacpp.gguf \
    --quantized-output ./asr_formal_merged-llamacpp-Q4_K_M.gguf
fi
